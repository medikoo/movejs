...
var ensureString     = require('es5-ext/object/validate-stringifiable-value')
  ...
  , debug            = require('debug-ext')('movejs')
...
module.exports = function (source, dest) {
	...
	return deferred(lstat(source), resolveRoot(dirname(source)), lstat(dest).then(function () {
		...
		return isModuleDirIndex(source)(function (isSourceDirIndex) {
			var sourceExt = extname(source), sourceDir = dirname(source)
			  , rootPrefixLength = root.length + 1
			  , dirReader, filePromises, isSourceExtRequired, isDestExtRequired, isSourceDirShadowed;

			return deferred(
				// 1. Rename module, and update require paths within it
				readFile(source)(function (code) {
					var sourceDirPosix = toPosix(sourceDir), ext = extname(source)
				...
				deferred(
					...
					isSourceDirIndex && isDirShadowed(sourceDir)(function (result) {
